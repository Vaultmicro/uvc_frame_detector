...
void ControlConfig::set_frame_format(const std::string& format) {
    ...
    if (format_lower == "mjpeg" || format_lower == "h264" || format_lower == "yuyv" || format_lower == "rgb") {
        frame_format = format_lower;
    } else {
        std::cerr << "Unsupported frame format: " << format << ". Using default format 'mjpeg'." << std::endl;
        frame_format = "mjpeg";
    }
}

int ControlConfig::get_width() {
    return width;
}

int ControlConfig::get_height() {
    return height;
}

int ControlConfig::get_fps() {
    return fps;
}

std::string ControlConfig::get_frame_format() {
    return frame_format;
}
