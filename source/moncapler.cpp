...
typedef struct __attribute__((packed, aligned(1))) {
    ...
    uint32_t iso_descriptor_number;   // Number of ISO Descriptor (4 bytes, 32 bits)
} URB_Data;
...
void log_packet_xxd_format(std::ofstream *log_file, const u_char *data, int length, int base_address = 0) {
    for (int i = 0; i < length; i += 16) {
        // Print address in hexadecimal
        *log_file << std::setw(8) << std::setfill('0') << std::hex << base_address + i << ": ";

        // Print hex values
        for (int j = 0; j < 16; ++j) {
            if (i + j < length) {
                *log_file << std::setw(2) << std::setfill('0') << std::hex << static_cast<int>(data[i + j]) << " ";
            } else {
                *log_file << "   ";  // For aligning ASCII section if data is less than 16 bytes
            }
        }
...
void print_capture_statistics(const pcap_stat &stats, 
                                unsigned int total_packet_count, unsigned long long total_packet_length, unsigned long long total_captured_length, 
                                unsigned int filtered_packet_count, unsigned long long filtered_total_packet_length, unsigned long long filtered_total_captured_length, 
                                std::ofstream *log_file = nullptr) {
    
    coutnlog("\n", log_file);
    coutnlog("Capture Statistics:", log_file);
    coutnlog("\n", log_file);
    coutnlog("Total Packets received in usbmon: " + std::to_string(stats.ps_recv), log_file);
    coutnlog("Total Packets dropped by system buffer: " + std::to_string(stats.ps_drop), log_file);
    coutnlog("Total Packets dropped by interface: " + std::to_string(stats.ps_ifdrop), log_file);
    coutnlog("\n", log_file);
    coutnlog("Total Packets counted in usbmon by application: " + std::to_string(total_packet_count), log_file);
    coutnlog("Total Packet Length bytes: " + std::to_string(total_packet_length), log_file);
    coutnlog("Total Captured Packet Length bytes: " + std::to_string(total_captured_length), log_file);
    coutnlog("\n", log_file);
    coutnlog("Filtered Packets (busnum=" + std::to_string(target_busnum) + ", devnum=" + std::to_string(target_devnum) + "): " + std::to_string(filtered_packet_count), log_file);
    coutnlog("Filtered Packet Length bytes (busnum=" + std::to_string(target_busnum) + ", devnum=" + std::to_string(target_devnum) + "): " + std::to_string(filtered_total_packet_length), log_file);
    coutnlog("Filtered Captured Packet Length bytes (busnum=" + std::to_string(target_busnum) + ", devnum=" + std::to_string(target_devnum) + "): " + std::to_string(filtered_total_captured_length), log_file);
    coutnlog("\n", log_file);
}
...
void clean_exit(int signum) {
    ...
    if (handle != nullptr) {
        ...
        if (pcap_stats(handle, &stats) >= 0) {
            ...
            print_capture_statistics(stats, total_packet_count, total_packet_length, total_captured_length, 
                                        filtered_packet_count, filtered_total_packet_length, filtered_total_captured_length,
                                         &log_file);
        } else {
...
void packet_handler(u_char *user_data, const struct pcap_pkthdr *pkthdr, const u_char *packet) {
    ...
    int endpoint_number = static_cast<int>(urb_data->endpoint & 0x7F); // Extract lower 7 bits for endpoint number
    //string direction = (mon_hdr->epnum & 0x80) ? "Device to Host" : "Host to Device";
    ...
    if (bus_number == target_busnum && device_address == target_devnum || 
        ...
        if (urb_data->urb_transfer_type == 0x02 || urb_data -> urb_transfer_type == 0x01) { // Control Transfer Type (0x02)
            // std::cerr << "Control || Interrupt Transfer detected, skipping this packet" << std::endl;
        ...
        // Isochronous Transfer (0x00)
            ...
            if (urb_data->iso_descriptor_number > 0) {
                std::vector<ISO_Descriptor> iso_descriptors(urb_data->iso_descriptor_number);
                ...
                for (uint8_t i = 0; i < urb_data->iso_descriptor_number; ++i) {
                    std::memcpy(&iso_descriptors[i], iso_descriptor_start + i * sizeof(ISO_Descriptor), sizeof(ISO_Descriptor));
                }

                for (uint8_t i = 0; i < urb_data->iso_descriptor_number; ++i) {
                    uint32_t start_offset = sizeof(URB_Data) + (16 * urb_data->iso_descriptor_number) + iso_descriptors[i].iso_descriptor_offset;
                    uint32_t end_offset = start_offset + iso_descriptors[i].iso_descriptor_length;
                    ...
                    temp_buffer.insert(temp_buffer.end(), packet + start_offset, packet + end_offset);
                    {
                        std::lock_guard<std::mutex> lock(queue_mutex);
                        packet_queue.push(temp_buffer);
                    }
                ...
                std::cerr << "No iso descriptor detected, skipping this packet" << std::endl;
                return;
            }

        } else {
            std::cerr << "Unknown transfer type detected, skipping this packet" << std::endl;
            return;
        }
        ...
        std::strftime(chrono_time_buffer, sizeof(chrono_time_buffer), "%Y-%m-%d %H:%M:%S", &local_tm);

        // Log packet information if it matches the target busnum and devnum
        *log_file << "Packet: " << filtered_packet_count 
                  << " Raw time: " << urb_data->urb_sec_hex << "." << urb_data->urb_usec_hex
                  << " Timestamp: " << kst_time
                  << " Chrono time: " << chrono_time_buffer << "." << std::setfill('0') << std::setw(3) << now_ms.count() 
                  << " [busnum: " << bus_number << ", devnum: " << device_address << ", epnum: " << endpoint_number << "]" << std::endl;
...
void process_packets(){
    ...
    while (true) { 
        ...
        queue_cv.wait(lock, []{ return !packet_queue.empty() || stop_processing; });
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i += 2) {
        if (std::strcmp(argv[i], "-in") == 0 && i + 1 < argc) {
            ...
            std::cerr << "Usage: " << argv[0] << " [-in usbmonX] [-bs buffer_size] [-bn busnum] [-dn devnum]" << std::endl;
            return 1;
        }
    }
    ...
    if (device == nullptr) {
        std::cerr << "Error: Device " << selected_device << " not found" << std::endl;
