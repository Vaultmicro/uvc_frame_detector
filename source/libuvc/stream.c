...
void _uvc_process_payload(uvc_stream_handle_t *strmh, uint8_t *payload, size_t payload_len) {
    ...
    /* 원시 데이터를 파일에 저장 */
    if (raw_data_file && payload_len > 0) {
        for (size_t i = 0; i < payload_len; i++) {
            fprintf(raw_data_file, "%02X ", payload[i]);
...
void LIBUSB_CALL _uvc_stream_callback(struct libusb_transfer *transfer) {
    ...
    // strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", timeinfo); // 시간 형식 설정
    ...
    case LIBUSB_TRANSFER_COMPLETED:
        if (transfer->num_iso_packets == 0) {
            /* Bulk transfer */
            // printf("Bulk transfer received, length: %d\n", transfer->actual_length);
            // fprintf(file, "Bulk transfer received, length: %d\n", transfer->actual_length);
            // for (int i = 0; i < transfer->actual_length; i++) {
            //     printf("%02x ", transfer->buffer[i]);
            //     fprintf(file, "%02x ", transfer->buffer[i]);
            //     if ((i + 1) % 16 == 0) {
            //         printf("\n");
            //         fprintf(file, "\n");
            //     }
            ...
            uint8_t *temp_buffer = (uint8_t *)malloc(transfer->actual_length);
            if (temp_buffer == NULL) {
                fprintf(stderr, "Memory allocation failed\n");
                return;
            }
            memcpy(temp_buffer, transfer->buffer, transfer->actual_length);

            // Add to queue (assuming you have a thread-safe way to handle this)
            pthread_mutex_lock(&queue_mutex);
            packet_queue[queue_rear] = temp_buffer;
            packet_sizes[queue_rear] = transfer->actual_length;
            ...
            for (packet_id = 0; packet_id < transfer->num_iso_packets; ++packet_id) {
                ...
                // printf("Isochronous packet %d received, length: %d\n", packet_id, pkt->actual_length);
                // fprintf(file, "Isochronous packet %d received, length: %d\n", packet_id, pkt->actual_length);
                // for (int i = 0; i < pkt->actual_length; i++) {
                //     printf("%02x ", pktbuf[i]);
                //     fprintf(file, "%02x ", pktbuf[i]);
                //     if ((i + 1) % 16 == 0) {
                //         printf("\n");
                //         fprintf(file, "\n");
                //     }
                ...
                printf("Isochronous packet %d received, length: %d\n", packet_id, pkt->actual_length);
                uint8_t *temp_buffer = (uint8_t *)malloc(pkt->actual_length);
                if (temp_buffer == NULL) {
                    fprintf(stderr, "Memory allocation failed\n");
                    return;
                }
                memcpy(temp_buffer, pktbuf, pkt->actual_length);

                pthread_mutex_lock(&queue_mutex);
                packet_queue[queue_rear] = temp_buffer;
                packet_sizes[queue_rear] = pkt->actual_length;
                queue_rear = (queue_rear + 1) % MAX_QUEUE_SIZE;
                pthread_cond_signal(&queue_cv);
                pthread_mutex_unlock(&queue_mutex);
